...
<script setup lang="ts">
  ...
  const { form, formRef, resetForm, validateForm, setFormValue } = useForm<RoleForm>(defaultValues, rules)
  ...
  const permissionTableData = vueRef<PermissionRow[]>([])
    ...
    (newRole) => {
      if (newRole) {
        setFormValue({
          ...
          statusEnabled: newRole.status === '启用',
          sort: newRole.sort,
        })
      } else {
        resetForm()
