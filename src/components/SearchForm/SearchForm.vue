  ...
  for (const field of props.fields) {
    if (field.type === 'select' && field.loadOptions) {
      ...
      try {
        ...
        logger.error(`Failed to load options for ${field.prop}:`, error)
...
function saveColumnVisibility() {
  ...
  try {
    ...
    logger.error('Failed to save column config to localStorage:', error)
  }
...
function emitVisibleFields() {
  const visible = props.fields.filter((field) => fieldVisibility[field.prop] !== false).map((field) => field.prop)
...
function handleColumnChange() {
  props.fields.forEach((field) => {
    if (fieldVisibility[field.prop] === false) {
      formData[field.prop] = props.defaultValues?.[field.prop] ?? ''
    }
  })
...
function handleSearch() {
  ...
  visibleFields.value.forEach((field) => {
    ...
    if (value !== undefined && value !== null && value !== '' && (!Array.isArray(value) || value.length > 0)) {
      visibleData[field.prop] = value
    }
  })
  emit('search', visibleData)
}
