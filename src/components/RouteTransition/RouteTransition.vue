<template>
  <router-view v-slot="{ Component, route }">
    ...
    >
      <keep-alive v-if="shouldKeepAlive(route)" :include="cachedViews">
        <component v-if="Component" :is="Component" :key="componentKey" v-bind="$attrs" />
      </keep-alive>
      <component v-else-if="Component" :is="Component" :key="componentKey" v-bind="$attrs" />
    </transition>
  </router-view>
</template>
...
<script setup lang="ts">
  ...
  const shouldKeepAlive = (route: RouteLocationNormalized): boolean => {
